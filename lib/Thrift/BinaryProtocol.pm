...
sub writeString{
    ...
    # Encode all strings as utf8 so that I can decode them as well
    $value = Encode::encode_utf8($value) if defined $value && length $value ;
...
sub readString
{
    ...
    if ($len) {
        my $string = $self->{trans}->readAll($len);

        # Decode all strings as utf8
        my $decoded = eval { Encode::decode_utf8($string) };
        if ($@) {
            print STDERR "Thrift::BinaryProtocol->readString() failed to decode_utf8() the string\n";
            $decoded = $string;
        }
        $$value = $decoded;
    }
